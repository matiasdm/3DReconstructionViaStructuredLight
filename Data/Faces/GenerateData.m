...
for id = ids;
    close all
    D      = .1*mean(double(imread(['GroundTruth_' num2str(id) '.png'])),3);
    [m,n]  = size(D);
    ...
    for k = 1:8;
        ...
        imwrite(uint8(cat(3,I,I,I)),...
            ['BinaryCodedStripes/Stripes_BinCoded_N8_' num2str(k) '_' num2str(id) '.bmp'],'bmp');
    ...
    for k = 1:3;
        ...
        imwrite(uint8(cat(3,I,I,I)),...
            ['Sin/Sin' num2str(k) '_T100_' num2str(id) '.bmp'],'bmp');
    ...
    imwrite(uint8(cat(3,I,I,I)),...
        ['Takeda/Stripes_Tak_strW_4_' num2str(id) '.bmp'],'bmp');
    
    %% C) Simulate D3D;
    % I) Sum,
    P = mean(double(imread(['D3D/Stripes_Sum_strW_4_p.bmp'])),3);
    I = SimulateProjection(P,D);
    imwrite(uint8(cat(3,I,I,I)),['D3D/Stripes_Sum_strW_4_' num2str(id) '.bmp'],'bmp');
    
    % II) Prod
    P = mean(double(imread(['D3D/Stripes_Pro_strW_4_p.bmp'])),3);
    I = SimulateProjection(P,D);
    imwrite(uint8(cat(3,I,I,I)),['D3D/Stripes_Pro_strW_4_' num2str(id) '.bmp'],'bmp');
    
    % III) Color
    P = double(imread(['D3D/Stripes_Color_strW_4_p.bmp']));
    I = SimulateProjection(P,D);
    imwrite(uint8(I),['D3D/Stripes_Color_strW_4_' num2str(id) '.bmp'],'bmp');
